...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    if (!empty($password)) {
        ...
        $hashed_password = $user['password']; // Use existing password if no new password is provided
    }
        ...
        if (in_array($profile_picture['type'], $allowed_types)) {
            if ($profile_picture['size'] <= 15 * 1024 * 1024) { // 15 MB
                ...
                $_SESSION['messageType'] = 'error';
                 header("Location: editProvider.php?id=$id");
                exit;
            }
        } else {
    ...
    $sql = "UPDATE users_patients SET firstname = ?, lastname = ?, email = ?, phone = ?, date = ?, address = ?, profile_picture = ?, username = ?, password = ?, state = ?, city = ?, zip = ?, service = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssssssssssi", $firstname, $lastname, $email, $phone, $date_of_birth, $address, $profile_picture_path, $username, $hashed_password, $state, $city, $zip, $service, $id);

    if ($stmt->execute()) {
        $_SESSION['message'] = "Staff record updated successfully.";
        $_SESSION['messageType'] = 'success';
    } else {
        $_SESSION['message'] = "Error updating staff record: " . $stmt->error;
        $_SESSION['messageType'] = 'error';
    }
...
if ($id) {
    $sql = "SELECT id, firstname, lastname, email, phone, date, address, profile_picture, username, password, state, city, zip, service FROM users_patients WHERE id = ?";
    $stmt = $conn->prepare($sql);
                                ...
                                <div class="card">
                                    <div class="card-body card-breadcrumb">
                                        <div class="page-title-box d-flex align-items-center justify-content-between">
                                            <h4 class="mb-0">Edit Provider information</h4>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="col-md-6">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control" name="date_of_birth" id="formrow-date-input" value="<?php echo htmlspecialchars($user['date']); ?>"  >
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="service" id="formrow-address-input" placeholder="" value="<?php echo htmlspecialchars($user['service']); ?>"  >
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3">
                ...
                <a href="provider.php" class="btn btn-secondary">Cancel</a>
            </div>
    </div>
